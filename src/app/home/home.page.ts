...
export class HomePage implements OnInit {
  ...
  async loadNearbyPlaces() {
    ...
    try {
      ...
      this.currentCity = 'Sua localização';

      const geoapifyCategories = this.getGeoapifyCategories();
      ...
      this.places = data.features.map((feature: any) => {
        const properties = feature.properties;

        return {
          id: properties.place_id || properties.datasource?.raw?.osm_id?.toString(),
          name: properties.name || 'Local sem nome',
          category: this.getCategoryFromGeoapify(properties.categories),
          image: this.getImageByCategory(properties.categories),
          rating: 4.5,
          reviews: 0,
          distance: this.formatDistance(properties.distance),
          openUntil: this.getOpenUntil(properties),
          favorite: false
        };
      });
      ...
      console.error(error);
      alert('Não foi possível carregar os lugares próximos.');
